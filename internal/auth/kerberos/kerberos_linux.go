...
// credential cache and generates a SPNEGO token for the given service principal name.
// The token is set as an "Authorization: Negotiate <token>" header on the request.
//
...
func SetSPNEGOHeader(req *http.Request, spn string) error {
	...
	encoded := base64.StdEncoding.EncodeToString(token.Bytes())
